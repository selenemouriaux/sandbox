 ...
 */
function prepareAndExecute(string $sql, array $values = []): array
{
    ...
    // Exécution 
    $status = $pdoStatement->execute($values);

    // On retourne l'objet PDOStatement
    return ['pdoStatement' => $pdoStatement, 'status' => $status];
}
...
function getOneResult(string $sql, array $values = []): array
{
    ...
    $pdoStatement = prepareAndExecute($sql, $values)['pdoStatement'];
